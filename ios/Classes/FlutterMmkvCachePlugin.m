#import "FlutterMmkvCachePlugin.h"
#import <MMKV/MMKV.h>

#define KEY @"key"
#define VALUE @"value"

@interface FlutterMmkvCachePlugin()
@property (strong,nonatomic) MMKV *mmkv;
@end

@implementation FlutterMmkvCachePlugin
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName:@"flutter_mmkv_cache"
                                     binaryMessenger:[registrar messenger]];
    FlutterMmkvCachePlugin* instance = [[FlutterMmkvCachePlugin alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if([method isEqualToString:@"setBool"]){
        ...
        result(res);
    } else if([method isEqualToString:@"getValueSizeForKey"]){
        NSString *key = arguments[KEY];
        size_t res = [self.mmkv getValueSizeForKey:key];
        result([NSNumber numberWithUnsignedLong:res]);
        ...
        NSMutableArray<NSString*>* keys = [NSMutableArray arrayWithCapacity:[self.mmkv count]];
        [self.mmkv enumerateKeys:^(NSString *key, BOOL *stop) {
            [keys addObject:key];
        }];
        result(keys);
    }
